...
UniValue gobject(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp  ||
        ...
        throw std::runtime_error(
                ...
                "  vote-conf          - Vote on a governance object by masternode configured in hrgold.conf\n"
    ...
    */
    ...
    if(strCommand == "vote-conf")
    {
        ...
        if(!fMnFound) {
            ...
            resultsObj.push_back(Pair("hrgold.conf", statusObj));
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* HrGold features */
    { "hrgold",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "hrgold",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "hrgold",               "gobject",                &gobject,                true,  {} },
    { "hrgold",               "voteraw",                &voteraw,                true,  {} },
